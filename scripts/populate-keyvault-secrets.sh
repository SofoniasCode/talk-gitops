...
TALK_PUBLIC_HOST="${TALK_PUBLIC_HOST:?Set TALK_PUBLIC_HOST (e.g. talk.dev.telodev.com)}"
TALK_ZITADEL_HOST="${TALK_ZITADEL_HOST:?Set TALK_ZITADEL_HOST (e.g. zitadel.dev.telodev.com)}"
TALK_ENV="${TALK_ENV:-dev}"
SECRET_PREFIX="talk-${TALK_ENV}"
...
set_secret "${SECRET_PREFIX}-zitadel-masterkey"       "$ZITADEL_MASTERKEY"
set_secret "${SECRET_PREFIX}-zitadel-database-dsn"    "$PG_DSN"
set_secret "${SECRET_PREFIX}-zitadel-database-host"   "$TALK_POSTGRES_FQDN"
set_secret "${SECRET_PREFIX}-zitadel-database-port"   "5432"
set_secret "${SECRET_PREFIX}-zitadel-database-name"   "t_zitadel"
set_secret "${SECRET_PREFIX}-zitadel-database-user"   "$TALK_POSTGRES_ADMIN_USER"
set_secret "${SECRET_PREFIX}-zitadel-admin-password"  "$ZITADEL_ADMIN_PASSWORD"

# Authz database
set_secret "${SECRET_PREFIX}-authz-database-url" "$AUTHZ_DB_URL"

# Identity webhook
set_secret "${SECRET_PREFIX}-identity-sync-zitadel-webhook-signing-key" "$WEBHOOK_SIGNING_KEY"

# Zitadel issuer / admin (placeholders until Zitadel is provisioned)
set_secret "${SECRET_PREFIX}-zitadel-issuer-url"       "https://${TALK_ZITADEL_HOST}"
set_secret "${SECRET_PREFIX}-zitadel-admin-token"      "PLACEHOLDER_RUN_PROVISION_SCRIPT"
set_secret "${SECRET_PREFIX}-zitadel-api-host-header"  "$TALK_ZITADEL_HOST"
set_secret "${SECRET_PREFIX}-zitadel-project-id"       "PLACEHOLDER_RUN_PROVISION_SCRIPT"

# oauth2-proxy
set_secret "${SECRET_PREFIX}-oauth2-proxy-client-id"                         "PLACEHOLDER_RUN_PROVISION_SCRIPT"
set_secret "${SECRET_PREFIX}-oauth2-proxy-client-secret"                     "PLACEHOLDER_RUN_PROVISION_SCRIPT"
set_secret "${SECRET_PREFIX}-oauth2-proxy-cookie-secret"                     "$OAUTH2_COOKIE_SECRET"
set_secret "${SECRET_PREFIX}-oauth2-proxy-redirect-url"                      "https://${TALK_PUBLIC_HOST}/oauth2/callback"
set_secret "${SECRET_PREFIX}-oauth2-proxy-cookie-domain"                     "$TALK_PUBLIC_HOST"
set_secret "${SECRET_PREFIX}-oauth2-proxy-whitelist-domain"                  ".${TALK_PUBLIC_HOST}"
set_secret "${SECRET_PREFIX}-oauth2-proxy-cookie-secure"                     "true"
set_secret "${SECRET_PREFIX}-oauth2-proxy-insecure-skip-issuer-verification" "false"
...
echo ""
echo "After Zitadel is running, run provision-zitadel-management.py with --write-azure-keyvault"
echo "to automatically update the placeholder secrets."
